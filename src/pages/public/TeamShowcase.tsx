...
const TeamShowcase: React.FC = () => {
  ...
  const getBio = (user: User): string => {
    ...
    if (name === "Bhushan Barun Mallick") {
      const adminBio =
        "Visionary leader and tech enthusiast, driving innovation and fostering collaboration in the tech community. Passionate about creating platforms that empower developers and bring ideas to life.";
    ...
    const fallbackBio =
      bios[name] ||
      "Dedicated team member contributing to INFI X TECH's mission of innovation and excellence.";
        ...
        if (!adminSnapshot.empty) {
          ...
          adminUser = {
            ...
            personalDetails: {
              ...
              name: adminData.personalDetails?.name || "Bhushan Barun Mallick",
              bio:
                adminData.personalDetails?.bio ||
                "Visionary leader and tech enthusiast, driving innovation and fostering collaboration in the tech community. Passionate about creating platforms that empower developers and bring ideas to life.",
            },
            educationalDetails: {
              ...adminData.educationalDetails,
            },
          ...
          console.log("Admin social links from Firebase:", adminUser.socialLinks);
        }
      } catch (adminError) {
  ...
  useEffect(() => {
    ...
    // Set up periodic refresh to catch profile updates
    const refreshInterval = setInterval(() => {
      console.log("🔄 Refreshing team data to catch profile updates...");
      fetchTeamMembers();
    }, 30000); // Refresh every 30 seconds

    // Listen for focus events to refresh when user returns to tab
    const handleFocus = () => {
      console.log("🔄 Tab focused, refreshing team data...");
      fetchTeamMembers();
    };
                              ...
                              portfolio: {
                                icon: (
                                  <svg
                                    ...
                                    viewBox="0 0 24 24"
                                  >
                                    <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zM4 12c0-.61.08-1.21.21-1.78L9.99 16v1c0 1.1.9 2 2 2v1.93C7.06 20.44 4 16.63 4 12zm13.89 5.4c-.26-.81-1-1.4-1.89-1.4h-1v-3c0-.55-.45-1-1-1h-6v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41C17.92 5.77 20 8.65 20 12c0 2.08-.81 3.98-2.11 5.4z"/>
                                  </svg>
                                ),
